...
@RelationBuilder
public class ClassGroupWithPercentageOfClientsDoingCastsAtLeastOne implements IRelationBuilder<MClass, MSystem> {
	...
	public Group<MClass> buildGroup(MSystem arg0) {
		...
		for (MClass type : systemTypes) {
			if (type.percentageOfClientsDoingCastsToMyDescendants() >= 1) {
				types.add(type);
			}
		}
		return types;
